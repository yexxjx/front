...
titledom.innerHTML="DOM조작 성공!"; // 2. 특정한 마크업 내 텍스트 .innerHTML, <마크업> 여기가inner</마크업>
...
function func3(){
    ...
    // DOM객체. style. CSS속성명=CSS속성값 // CSS는 - 하이픈 가능, js는 불가능
    // 속성명은 카멜표기법 사용하기
...
function func7(){
    let mainImageDom=document.querySelector("#mainImage");
    // Dom객체명.src="이미지경로"
    mainImageDom.src="https://placehold.co/600x400/";
}

/* 예제 8 : select 에서 선택한 option값 console 출력하기. */
// <마크업 onclick="함수명()"/> 클릭 했을 때
// <마크업 onchange="함수명()"/> 값이 변경 됐을 때
function func8(){
    let foodSelectDom=document.querySelector("#foodSelect");
    let food=foodSelectDom.value; // 선택된 option 값 가져오기
    console.log(food);
}
...
let products=[ ];
function func9(){
    let productInputDom=document.querySelector("#productInput");
    let product=productInputDom.value;
    products.push(product);
    let productPrintDom=document.querySelector("#productPrint");
