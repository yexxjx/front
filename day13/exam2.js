...
// [매개변수] 무엇을 넣고 [명령어] 무엇을 처리 [반환값] 무엇을 반환하지?
...
// [매개변수]name, [명령어]x, [반환값]"환영합니다, [이름]님!"
function getWelcomeMessage(name){ // name 매개변수 라고 한다. 매개변수명은 아무거나로 정의
    return `환영합니다, ${name}님!`;
}
...
// [매개변수]x, [명령어]counter값 1씩 증가, [반환값]x
let counter=0;
...
// [매개변수]이름, [명령어]userList 전역 배열에 push, [반환값]
// let userList=[]; // 전역변수
// function addUser(name1){
//     userList.push(name1); // name1 : 매개변수
// }
// let name2=prompt("이름 입력"); // name2 : 전역변수, 함수 밖 변수명이 동일한 경우 있음
// addUser(name2);
...
function printLocal(){
    let localText='지역';
    console.log(globalText); // 즉) {} 중괄호 안으로 들어가는 건 가능
    console.log(localText); // {} 중괄호 밖으로 나가는 건 불가능
}
console.log(globalText);
// console.log(globalText,localText); // [오류] localText is not defined