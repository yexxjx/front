...
// [2] 각 입력받은 값들이 저장된 변수들을 속성값에 대입하여 객체 선언 {"속성명":변수값}
...
let member={id:아이디, pw:비밀번호, name:이름};

// ================[*] 만약에 입력받은 아이디가 배열 내 존재하면 (경우의 수 발생)
let find=false; // 중복 여부 체크 변수, true 중복 false 중복 아님, 스위치변수=on/off=참/거짓=맞다/틀리다
for(let index=0; index<=members.length-1;index++){ // [*] 배열 내 모든 객체를 하나씩 조회 == 배열 순회(0부터 마지막 인덱스)
    let m=members[index]; // index번째 회원정보(객체) 꺼내기
    if(m.id==member.id){ // 만약에 index번째 회원정보의 아이디가 입력받은 아이디와 같으면
        // console.log("중복 입니다.");  불가능, 아직 뒤에 있는 인덱스까지 확인 안했으므로 최종 결론 불가능
        // 최종 결론 내지 말고 중복을 찾았다 못찾았다 기록, 결론은 반복문 끝나고 하기
        find=true; // 찾았다고 기록하고
        break; // 반복문 탈출
    }
}
if(find==false){ // 중복이 없으면
members.push(member); // [3] 배열 내 생성한 객체 저장, 배열변수명.push(새로운객체)
...
for(let index=0; index<=scores.length-1; index++){
    sum+=scores[index].math; // index번째의 수학 점수를 누적 합계 하기
}
